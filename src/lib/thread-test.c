...
_start_event_handler() {
  ...
  while ( !( event->event_handler_state & EVENT_HANDLER_STOP ) ) {
    new_execute_timer_events( &timeout_usec );
...
bool ( *new_writable )( int fd ) = _writable;
